...
import { getAnimationState } from "../../../../utils/sanitizer/animation/animationState";
...
export function GameFilterSideBar(props: GameFilterSideBarProps) {
  ...
  const [priceMin, setPriceMin] = useState(
    filters.price_min ? String(filters.price_min) : "0.00",
  );
  const [priceMax, setPriceMax] = useState(
    filters.price_max ? String(filters.price_max) : "0.00",
  );
  const [launchDateFrom, setLaunchDateFrom] = useState(
    filters.launch_date_from ?? "",
  );
  const [launchDateTo, setLaunchDateTo] = useState(
  ...
  function handleConfirmPrice() {
    filters.onConfirmPrice?.(priceMin, priceMax);
  }
  function handleConfirmLaunchDate() {
    filters.onConfirmLaunchDate?.(launchDateFrom, launchDateTo);
  }
      ...
      <div className="w-full flex justify-center">
        ...
        >
          ...
          <Button
            ...
            as="button"
            variant="transparent"
            onClick={() => filters.onChangeActive("true")}
            className={`max-lg:active:bg-white max-lg:active:text-night lg:hover:bg-white lg:hover:text-night transition-all duration-300 text-left w-full flex justify-between items-center gap-2 px-4 py-2 text-sm`}
          >
            Sim
            <span>
          ...
          <Button
            ...
            as="button"
            variant="transparent"
            onClick={() => filters.onChangeActive("false")}
            className={`max-lg:active:bg-white max-lg:active:text-night lg:hover:bg-white lg:hover:text-night transition-all duration-300 text-left w-full flex justify-between items-center gap-2 px-4 py-2 text-sm`}
          >
            Não
            <span>
