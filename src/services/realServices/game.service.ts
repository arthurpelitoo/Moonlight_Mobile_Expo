...
import type {
    GamePaginatedQueryPayload,
    GamePayload,
} from "../../@types/game/game.payload";
import { sanitizeData } from "../../utils/sanitizer/sanitizer";
import { api } from "../api";
import {
    fetchGameByIdMock,
    fetchGamesPaginatedMock,
} from "../fakeServices/game.fakeservice";
...
export async function fetchGamesPaginatedAdmin(
  query: GamePaginatedQueryPayload,
): Promise<PaginatedResponse<GameResponseDTO>> {
  if (process.env.EXPO_USE_MOCK === "true") {
    return fetchGamesPaginatedMock(query.page, query.limit);
...
export async function fetchGamesByTitle(
  title: string,
): Promise<GameResponseDTO[] | GameResponseDTO> {
